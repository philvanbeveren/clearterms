...
const styles = StyleSheet.create({
  ...
  header: { marginBottom: 14, paddingBottom: 10, borderBottomWidth: 1, borderBottomColor: "#e5e5e5" },
  ...
  box: { borderWidth: 1, borderColor: "#e2e2e2", padding: 10, marginBottom: 10, borderRadius: 6 },
  line: { marginBottom: 6 },
  ...
  numberText: { flex: 1 },
  emailBox: { borderWidth: 1, borderColor: "#e2e2e2", padding: 10, marginBottom: 10, borderRadius: 6, backgroundColor: "#f7f7f7" },
  mono: { fontFamily: "Courier", fontSize: 10, lineHeight: 1.4 },
  riskRow: { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 2 },
  pill: { paddingTop: 3, paddingBottom: 3, paddingLeft: 8, paddingRight: 8, borderRadius: 999, fontSize: 9, fontWeight: 700 },
...
const PDF_T = {
  nl: {
    ...
    footer: "Dit document is gegenereerd met een deterministische scenario-engine + AI-uitleg. Het is geen juridisch advies.",
    dash: "—",
  ...
  fr: {
    ...
    footer: "Ce document est généré à partir d’un moteur déterministe + une explication IA. Il ne constitue pas un avis juridique.",
    dash: "—",
  ...
  en: {
    ...
    footer: "This document is generated from a deterministic scenario engine + AI explanation. It does not provide legal advice.",
    dash: "—",
...
export async function renderClearTermsPdfBuffer(args: { ai: ExplainAIOutput; engine: any }) {
  const lang = normalizeLang(args.engine?.intake?.language ?? "en");
  const data = args.ai;
  ...
  const riskLevel = String(args.engine?.scenario?.riskLevel ?? "unknown").toLowerCase();
  const summary = data?.explanation?.plainSummary ?? "";

  const [logoDataUri, hrBullets] = await Promise.all([
    loadLogoDataUri(),
    generateHrReadyBullets({ lang, scenarioLabel, scenarioCode, noticeText, riskLevel, summary }),
  ]);

  const element = buildPdfElement(data, args.engine, lang, hrBullets, logoDataUri);
